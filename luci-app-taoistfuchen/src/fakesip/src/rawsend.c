    ...
    *modified = 0;
    ...
    if (sll->sll_pkttype == PACKET_HOST) {
        ...
        */
        ...
        /* The decoys have already been sent. Releasing the queued original now
         * preserves its checksum/GSO state, mark, priority, and other skb
         * metadata while still keeping the intended send order. */
        return NF_ACCEPT;
    } else {
