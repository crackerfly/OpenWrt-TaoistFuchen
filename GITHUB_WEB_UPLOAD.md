...
```text
OpenWrt-TaoistFuchen-v2.0.0-r2/.github/workflows/build.yml
```
...
```text
luci-app-taoistfuchen-2.0.0-r2.apk
```
...
`cgi-io`、`nftables`、`kmod-nft-queue`、`coreutils-stat`、`coreutils-od` 及递归依赖（例如
`kmod-nft-core`）。不要从 Actions artifact 手工安装一组依赖 APK。
