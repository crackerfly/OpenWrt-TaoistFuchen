...
ARCHIVE = ROOT / "third_party/sources/FakeSIP-TaoistFuchen-0.9.5.tar.gz"
...
def main() -> None:
    ...
    assert "FAKESIP_VERSION := 0.9.5" in wrapper
    ...
    assert "LUCI_BUILD_DEPENDS:=libnetfilter-queue attr acl libcap" in package_makefile

    provenance = text(ROOT / "THIRD_PARTY_SOURCES.md")
    assert "FakeSIP 0.9.5" in provenance
