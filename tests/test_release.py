...
def main() -> None:
    ...
    assert (
        ...
        "+coreutils-stat +coreutils-od +libnetfilter-queue"
    ) in makefile
    ...
    assert "FakeSIP 0.9.4" in sources
    assert "2a48dc7c1d61a582" in sources
    assert (ROOT / "third_party/sources/FakeHTTP-0.9.18.tar.gz").is_file()
    assert (ROOT / "third_party/sources/FakeSIP-TaoistFuchen-0.9.4.tar.gz").is_file()
    ...
    assert "FakeSIP 0.9.4" in fakesip_view
    ...
    for needle in ("FakeHTTP 0.9.18", "FakeSIP 0.9.4"):
    ...
    assert '"$module_count" -eq 9' in sdk_configurer
    for expected in (
        "libmnl=y",
