...
CONFIGURER="$ROOT/scripts/configure-openwrt-sdk.sh"
COLLECTOR="$ROOT/scripts/prepare-artifact.sh"
VERIFIER="$ROOT/scripts/verify-built-apk.py"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT INT TERM

for required in "$MINIMIZER" "$CONFIGURER" "$COLLECTOR" "$VERIFIER"; do
	if [ ! -f "$required" ]; then
...
grep -Fqx 'CONFIG_PACKAGE_libattr=y' "$CONFIGURER"
grep -Fqx 'CONFIG_PACKAGE_libacl=y' "$CONFIGURER"
grep -Fqx 'CONFIG_PACKAGE_libcap=y' "$CONFIGURER"
grep -Fq '[ "$module_count" -eq 12 ]' "$CONFIGURER"
grep -Fq 'libpthread=m' "$CONFIGURER"
grep -Fq 'librt=m' "$CONFIGURER"
...
config PACKAGE_libattr
	tristate
	default m

config PACKAGE_libacl
	tristate
	default m

config PACKAGE_libcap
	tristate
	default m
...
[ "$(default_for PACKAGE_libpthread)" = "n" ]
[ "$(default_for PACKAGE_librt)" = "n" ]
[ "$(default_for PACKAGE_libattr)" = "n" ]
[ "$(default_for PACKAGE_libacl)" = "n" ]
[ "$(default_for PACKAGE_libcap)" = "n" ]
...
payload.extend(
    ...
    b"FakeSIP version 0.9.5\0"
    b"  -0                 process outbound packets\n\0"
    b"  -1                 process inbound packets\n\0"
    b" (IPv4 only)\0"
    b" (IPv6 only)\0"
    b" (IPv4 and IPv6)\0"
    b" (outbound only)\0"
    b" (inbound only)\0"
    b" (inbound and outbound)\0"
...
MAIN_APK="$SDK/bin/packages/aarch64_cortex-a53/base/luci-app-taoistfuchen-2.1.0-r3.apk"
touch "$MAIN_APK" \
...
[ -f "$TMP/output-one/FakeSIP-TaoistFuchen-0.9.5.tar.gz" ]
[ ! -e "$TMP/output-one/FakeSIP-TaoistFuchen-0.9.4.tar.gz" ]
[ ! -e "$TMP/output-one/FakeSIP-Droid-MAX-0.9.3.tar.gz" ]
[ -f "$TMP/output-one/SHA256SUMS" ]

touch "$SDK/bin/packages/aarch64_cortex-a53/base/luci-app-taoistfuchen-2.1.0-r3-duplicate.apk"
...
rm "$SDK/bin/packages/aarch64_cortex-a53/base/luci-app-taoistfuchen-2.1.0-r3-duplicate.apk"

python3 - "$TMP/fakesip-stale-reporting.fixture" <<'PY'
import sys
from pathlib import Path
...
payload.extend(
    b"/lib/ld-musl-aarch64.so.1\0"
    b"FakeSIP version 0.9.5\0"
    b"  -0                 process inbound packets\n\0"
    b"  -1                 process outbound packets\n\0"
    b" (IPv4 only)\0"
    b" (IPv6 only)\0"
    b" (inbound only)\0"
    b" (outbound only)\0"
...
Path(sys.argv[1]).write_bytes(payload)
PY
if FAKE_APK_BINARY="$TMP/fakesip-stale-reporting.fixture" READELF="$TMP/bin/readelf" \
	sh "$COLLECTOR" "$SDK" "$TMP/output-stale-reporting" >"$TMP/stale-reporting.log" 2>&1; then
	echo "artifact collector accepted stale FakeSIP reporting" >&2
	exit 1
fi
grep -Fq 'missing fakesip payload marker' "$TMP/stale-reporting.log"
