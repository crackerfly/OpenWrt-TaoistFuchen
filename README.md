...
3. FakeSIP：在路由器 WAN 侧配置项目自维护的 FakeSIP 0.9.5。

FakeHTTP 是经过校验的静态 AArch64 ELF；FakeSIP 0.9.5 则由 OpenWrt SDK 从仓库内完整
...
本包自维护的 0.9.5 基于 Droid-MAX/FakeSIP commit
...
0.9.4 阶段修正了基线源码中的以下问题，0.9.5 完整保留这些数据包路径修复：
...
0.9.5 不改变 NFQUEUE 分类、rawsend 门控、规则或 init 参数映射，只修正表达层：帮助文本
现在将 `-0`/`-1` 正确说明为仅出站/仅入站，启动日志会明确显示仅出站、仅入站或双向，
并明确显示仅 IPv4、仅 IPv6 或 IPv4+IPv6。
...
基线版本的内部方向变量名与实际 `PACKET_HOST`/`PACKET_OUTGOING` 路径相反；0.9.5 保持
实际处理路径不变，init 脚本继续在仅入站、仅出站模式下补偿对应 CLI 标志，使页面、帮助
与运行日志都按路由器真实流向显示。
默认命令等价于：
...
IPv6 WAN 必须已经具备可用的全局 IPv6 连接；0.9.5 暂不解析 IPv6 扩展头，因此只有 UDP
...
开机延迟只作用于路由器开机时已经启用服务的自动启动。FakeSIP 默认 40 秒；FakeHTTP 默认 60 秒。
用户可在各自设置页修改为 0–600 秒；`0` 表示开机也不延迟。页面和命令行的人工启停立即执行，
不读取这段延迟；如果所选接口确实不存在，原有安全校验仍会拒绝启动。

- 两个等待任务并行计时，目标是系统启动后的第 40/60 秒，不会串行累计成 100 秒；
- 等待任务由独立 procd 调度器管理，状态与生命周期锁位于 root-only 的 `/var/run` 目录；
  不创建脱离 procd 的后台 `sleep`；
- 开机延迟到期前不验证尚未创建的 PPPoE 设备，WAN 提前 `ifup` 只记录设备状态，不能绕过
  剩余等待时间；到期时才使用最新配置和接口状态启动；
- WAN `ifdown` 使用专用 stop-only 路径：先使旧等待 token 失效，再停止实例并清理本服务
  nft 表，不在 PPP 重建过程中尝试重启；
- WAN `ifup` 使用独立恢复路径：开机窗口内保持等待，窗口结束后只在明确匹配且设备存在时
  立即恢复，不额外增加重拨延迟；
- 人工 start/restart/reload、调度器到期及 `ifdown`/`ifup` 均由每服务生命周期锁串行化，
  迟到 timer 或旧设备事件不能复活已停止服务；
- procd 主进程异常退出仍使用受限 respawn，不会重新套用开机延迟；
- 两个服务始终注册 UCI reload trigger，stdout/stderr 直接进入 OpenWrt `logd`，LuCI 日志
  面板读取系统 ring buffer，不创建可能写满 RAM 的 `/tmp/*.log`；
- 第一次安装或升级到 r3 会按已确认的迁移方案设置 40/60 秒并写入完成标记；同一 r3 重装
  会保留用户此后修改的延迟值。
...
- `flock`（用于串行化人工操作、开机 timer 与 WAN hotplug 生命周期）；
...
卸载会停止并禁用 Custom Logo、FakeHTTP、FakeSIP 和内部开机调度器这四个受管服务，移除
模板注入、运行时资源、staging 文件、旧版 cron 行及本工具创建的 nft 表；不会删除共享的
nftables、cron 或其他系统组件。

## 许可与对应源代码

- LuCI 应用代码：MIT；
- FakeHTTP 二进制与 FakeSIP 0.9.5 源码：GNU GPLv3；
